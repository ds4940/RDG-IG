  ...
  return (
    <>
      ...
      <Box
        ...
        minHeight="100vh"
      >
        <ImageList data-testid="ImageContainer" cols={4} gap={30}>
          {listOfImages.map((item) => {
            return (
              <ImageListItem key={item.Id} data-test-group="ImageListItem">
                <img
                  srcSet={`${item.Image}`}
                  src={`${item.Image}`}
                  alt={item.Keywords.join(", ")}
                  loading="lazy"
                  data-test-group="ImageListItemImg"
                />
                <ImageListItemBar
                  position="below"
                  title={item.Title}
                  subtitle={`keywords: ${item.Keywords}`}
                  data-test-group="ImageListItemBar"
                />
              </ImageListItem>
            );
          })}
